...
export const ScenarioAssessment: React.FC<ScenarioAssessmentProps> = ({
  ...
  const [content, setContent] = useState<ScenarioAssessmentContent | null>(
    null,
  );
  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        ...
        const newSessionId = createAssessmentSession(
          candidateId,
          skill,
          difficulty,
        );
        ...
        const assessmentContent = await generateScenarioAssessment(
          skill,
          difficulty,
        );
        ...
        skill,
        {
          onViolation: (violation) => {
            ...
            setCheatingEvents((prev) => [
              ...prev,
              `[PROCTORING] ${violation.description}`,
            ]);
  ...
  useEffect(() => {
    ...
    const handleContextMenu = (e: MouseEvent) => {
      ...
      setActiveAlert(
        "Security Warning: Right-click menu disabled during assessment.",
      );
    ...
    const handleKeyDownGlobal = (e: KeyboardEvent) => {
      ...
      if (
        ...
        setActiveAlert(
          "Security Violation: DevTools disabled during assessment.",
        );
        setTimeout(() => setActiveAlert(null), 3000);
      }
    ...
    const handleResize = () => {
      ...
      if (Math.abs(newWidth - lastWidth) > 200) {
        setCheatingEvents((prev) => [
          ...prev,
          `Window resized (${lastWidth} → ${newWidth})`,
        ]);
  ...
  // Re-attach video stream if step changes and ref mounts later
  useEffect(() => {
    ...
    const checkInterval = setInterval(() => {
      if (videoRef.current && videoRef.current.srcObject) {
        ...
        if (!videoTrack || !videoTrack.enabled || videoTrack.muted) {
          ...
          setActiveAlert(
            "Proctoring Alert: Please ensure your face is visible.",
          );
          setTimeout(() => setActiveAlert(null), 3000);
        }
      }
  ...
  const analyzeTypingSpeed = useCallback(
    (newText: string, questionId: number) => {
      if (!timerActive) return;
      ...
      if (charDifference > 10) {
        ...
        if (charDifference > 50 && timeSinceLastCheck < 500) {
          setCheatingEvents((prev) => [
            ...prev,
            `Rapid input detected: ${charDifference} chars in ${timeSinceLastCheck}ms`,
          ]);
      ...
      if (SpeechRecognitionAPI) {
        ...
        recognition.onresult = (event: any) => {
          ...
          if (finalText.trim()) {
            ...
            setAnswers((prev) => {
              const qId =
                content?.situationalQuestions[currentQuestionIndexRef.current]
                  ?.id;
              if (qId == null) return prev;
              return {
                ...prev,
                [qId]: ((prev[qId] || "") + " " + finalText).trim(),
              };
            });
          }
        };
        ...
        setOralTranscript(
          "(Live transcription not supported in this browser — your audio has been recorded and will be evaluated.)",
        );
      }
  ...
  const isCurrentQuestionAnswered = (): boolean => {
    ...
    if (
      ...
      !currentQuestion.requiresOralResponse &&
  ...
  const handleSubmit = async () => {
    ...
    if (unanswered.length > 0) {
      setActiveAlert(
        `Cannot submit: Please answer all questions. Unanswered: Question(s) ${unanswered.join(", ")}`,
      );
      setTimeout(() => setActiveAlert(null), 5000);
      return;
    }
  ...
  const handleComplete = async () => {
    ...
    onComplete({
      ...
      cheatingReason:
        hasCheatingEvents || hasLowIntegrity
      ...
      <div className="min-h-screen bg-cream flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">
            Generating scenario questions for {skill}...
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-cream">
            ...
            {proctoringActive && (
              ...
              >
                ...
                <span className="font-medium">
                  {webcamActive ? "Proctoring" : "No Camera"}
                </span>
              </div>
            )}
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <AnimatePresence mode="wait">
            ...
            >
              <div className="p-8 md:p-12">
                ...
                <p className="text-center text-gray-500 mb-8 max-w-lg mx-auto">
                  Read this context carefully. All following questions will be
                  based on this scenario.
                </p>

                <div className="bg-gradient-to-r from-orange-50 to-amber-50 rounded-xl p-6 mb-8">
                  <h3 className="font-bold text-orange-900 mb-3 flex items-center gap-2">
                    <span className="bg-white px-2 py-0.5 rounded text-xs border border-orange-200 shadow-sm">
                      Your Role
                    </span>
                    {content.title || skill}
                  </h3>
                  ...
                  <div className="grid md:grid-cols-2 gap-8">
                    <div>
                      ...
                      <ul className="space-y-3">
                        {content.situationalQuestions
                          .slice(0, 4)
                          .map((q, i) => (
                            <li
                              key={i}
                              className="flex gap-3 text-gray-600 text-sm"
                            >
          ...
          {step === "situational" &&
            content.situationalQuestions[currentQuestionIndex] && (
                  ...
                  <div className="p-4 border-b border-gray-100 bg-gray-50">
                    ...
                    <div className="w-full bg-gray-200 h-1.5 rounded-full mt-2 overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${(Object.keys(answers).length / content.situationalQuestions.length) * 100}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                          ...
                          <span className="flex items-center gap-2">
                            ...
                            >
                              {audioBlob ? (
                                ...
                                content.situationalQuestions.length + 1
                              )}
                            </span>
                      ...
                      {content.situationalQuestions[currentQuestionIndex]
                        ...
                        <div className="bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-200 rounded-xl p-4 mb-6">
                          <div className="flex items-center gap-2 mb-2">
                            <span className="bg-orange-100 text-orange-700 px-2 py-0.5 rounded text-xs font-bold uppercase tracking-wider">
                              Scenario
                            </span>
                          </div>
                          ...
                          >
                            {content.situationalQuestions[
                              ...
                              <button
                                ...
                                className={`w-full text-left p-4 rounded-xl border-2 transition flex items-center gap-3 group ${
                                  answers[
                                    content.situationalQuestions[
                                      currentQuestionIndex
                                    ].id
                          ...
                          <div className="flex-1 flex flex-col gap-4">
                            <div className="bg-purple-50 border border-purple-200 rounded-xl p-4 flex items-start gap-3">
                              ...
                              <p className="text-sm text-purple-800">
                                ...
                                <strong>verbal response</strong>. Press{" "}
                                <strong>Start Recording</strong>, speak your
                                answer clearly, then press <strong>Stop</strong>
                                .
                              </p>
                            </div>
                        ...
                        <>
                          ...
                          >
                            ...
                            <div className="flex justify-between text-xs text-gray-400">
                              ...
                              <span
                                className={`font-medium ${(answers[content.situationalQuestions[currentQuestionIndex].id]?.length || 0) >= 50 ? "text-green-600" : "text-gray-400"}`}
                              >
                                {answers[
                                  content.situationalQuestions[
                                    currentQuestionIndex
            ...
            >
              <div className="mb-6">
                ...
                <p className="text-gray-600">
                  Record your verbal response to the following scenario.
                </p>
              </div>
              ...
              <div className="bg-gray-50 rounded-xl p-4 mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {content.oralResponseTask.evaluationCriteria.map(
                    (criteria, idx) => (
                      <span
                        key={idx}
                ...
                <button
                  onClick={() => {
                    ...
                    if (content.situationalQuestions[currentQuestionIndex]) {
                      handleAnswerSelect(
                        content.situationalQuestions[currentQuestionIndex].id,
                        oralTranscript || "(Audio recorded)",
                      );
                    }
          ...
          {step === "results" && result && (
            ...
            >
              ...
              >
                <div
                  className={`w-24 h-24 ${getScoreBg(result.score)} rounded-full flex items-center justify-center mx-auto mb-4`}
                >
                  <span
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
                {Object.entries(result.categoryScores).map(
                  ([category, score]) => (
                    <div
                      key={category}
                      className={`${getScoreBg(score as number)} rounded-xl p-4 text-center`}
                    >
                      <p
              ...
              <div className="bg-gray-50 rounded-xl p-6 mb-6">
                ...
                <div className="space-y-4">
                  ...
                  {result.feedback
                    .split(
                      /(?=[A-Z][a-z]+\s?(?:Solving|Knowledge|Communication|Focus|Professionalism|Skills)?:)/g,
                    )
                    ...
                    .map((section, idx) => {
                      ...
                      const getCategoryColor = (cat: string | null) => {
                        ...
                        if (
                          ...
                          catLower.includes("positive")
                        )
                          return "bg-green-100 text-green-800 border-green-200";
                        if (
                          catLower.includes("problem") ||
                          catLower.includes("improvement") ||
                          catLower.includes("however")
                        )
                          return "bg-amber-100 text-amber-800 border-amber-200";
                        if (catLower.includes("communication"))
                          return "bg-blue-100 text-blue-800 border-blue-200";
                        if (catLower.includes("professional"))
                          return "bg-purple-100 text-purple-800 border-purple-200";
                        if (
                          catLower.includes("knowledge") ||
                          catLower.includes("role")
                        )
                          return "bg-indigo-100 text-indigo-800 border-indigo-200";
                        if (
                          catLower.includes("customer") ||
                          catLower.includes("focus")
                        )
                          return "bg-teal-100 text-teal-800 border-teal-200";
                        if (catLower.includes("verbal"))
                      ...
                      return (
                        <div
                          key={idx}
                          className={`${header ? "bg-white rounded-lg border p-4" : ""}`}
                        >
                          {header && (
                            <div className="flex items-center gap-2 mb-2">
                              <span
                                className={`px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wide border ${getCategoryColor(header)}`}
                              >
                                {header}
                              </span>
                            </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium text-gray-800 text-sm line-clamp-1">
                            {question.taskType === "multiple_choice"
                              ? "Multiple Choice"
                              : question.taskType === "oral_response"
                                ? "Oral Response"
                                : "Written Response"}
                            : Q{idx + 1}
                          </span>
