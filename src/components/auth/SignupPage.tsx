...
export const SignupPage: React.FC<{
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    ...
    // Google login always creates candidate accounts, regardless of role selection
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className={`p-4 rounded-xl border flex flex-col items-center gap-2 transition-all ${
                role === "candidate"
                  ? "border-orange bg-orange/5 text-orange ring-1 ring-orange shadow-sm"
            ...
            >
              <Code
                size={22}
                className={
                  role === "candidate" ? "text-orange" : "text-slate-500"
                }
              />
              <span
                className={`font-semibold text-sm ${role === "candidate" ? "text-orange" : "text-slate-700"}`}
              >
                Candidate
              </span>
            ...
            <button
              ...
              className={`p-4 rounded-xl border flex flex-col items-center gap-2 transition-all ${
                role === "employer"
                  ? "border-teal bg-teal/5 text-teal ring-1 ring-teal shadow-sm"
                  : "border-slate-200 hover:border-slate-300 bg-white hover:bg-slate-50"
              }`}
            >
              <Briefcase
                size={22}
                className={role === "employer" ? "text-teal" : "text-slate-500"}
              />
              <span
                className={`font-semibold text-sm ${role === "employer" ? "text-teal" : "text-slate-700"}`}
              >
                Employer
              </span>
              <span className="text-xs text-slate-500 font-medium">
